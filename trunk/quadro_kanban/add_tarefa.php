<?php
	...
	if ( isset($_GET['pro_id']) ) 
	{
		...
		if (isset($_POST['send'])) 
		{
			...
			// recupera os dados digitados no formulсrio
			...
			if ( !empty($tar_titulo) )
			{
				...
				// criando query de inserчуo na tabela tarefa
				...
				//execulta query de inserчуo na tabela tarefa
				$data = mysqli_query($dbc, $query)
					or die('Erro ao executar a inserчуo na tabela tarefa' );
					
				// recupera o id da terefa inserida e insere na tabela responsсvel
				$ultimo_tar_id = mysqli_insert_id($dbc);
				
				$query = "INSERT INTO `responsavel`(  `tar_id`, `usu_id`) VALUES ( '$ultimo_tar_id', '$usu_id_reponsavel' )"
				or die (  mysqli_error($dbc) );
				
				//execulta query de inserчуo na responsсvel
				$data = mysqli_query($dbc, $query)
					or die( mysqli_error($dbc) );
			}
			/* Fecha conexуo com o banco */
			mysqli_close($dbc);
		}
	}
